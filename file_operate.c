...
make_temp_dir(char* filename)
{
    ...
    /*analyse path information*/
    rev = distill_path(filename, pathname_1, pathname_2);
    if(rev)
        return rev;
    /*make temp directory for first download*/
    rev = make_dir(pathname_1);
    if(rev)
        return rev;
    /*make temp directory for second download*/
 ...
 *   RETURN VALUE:
 *       OK   : filename1 the same size and filename2
 *       other: filename1 the diffent size and filename2
 *   DESCRIPTION:
 *      compare files by the number of bytes
 *
 ******************************************************************/
int
 ...
 *             pathname_2: file name2
 *   RETURN VALUE:
 *       OK   : filename1 the same Character and filename2
 *       other: filename1 the diffent Character and filename2
 *   DESCRIPTION:
 *      compare files by the number of Character
 *
 ******************************************************************/
int
 ...
 *             pathname_2: file name2
 *   RETURN VALUE:
 *       OK   : filename1 and filename2 equal
...
file_compare(char* pathname_1, char* pathname_2)
{
    ...
    if(pathname_2 == NULL)
        return 0x29; /* memory or process's error occured */
    /*compare two files by file size*/
    rev = size_compare(pathname_1, pathname_2);
    if(rev)
        return rev;

    /*compare two files by binary*/
    rev = binary_compare(pathname_1, pathname_2);
    if(rev)
        return rev;

    /*delete the file of second-downloaded */
    rev = remove(pathname_2);
    if(rev)
    {
        write_log("[ERROR INFO] remove() error occured in file_compare()\n");
...
local_size(void)
{
    ...
    if(free_size <= (dir_size + 1024*1024))
        return 0x30; /*HDD space is not enough to save temp files*/
    return OK;
}
