 ...
 *   PARAMETER: NONE
 *   RETURN VALUE:
 *                 OK: execute file copy cancel
 *                 NG: can't execute file copy cancel
 *   DESCRIPTION:
...
file_copy_cancel(void)
{
    ...
    read_status(&status);
    /*process can execute cancel command when status is COPYSTART or COPYING*/
 ...
 *       cmd_category : COPYCMD, CANCELCMD, EXITCMD
 ...
 *   RETURN VALUE:
 *       Upon successful completion, info_return() shall return OK;
                                    ...
                                    unsigned char result , unsigned char code)
{
            ...
            case COPYCMD: /*copy command's return*/
                if(excute_flag == 2)
                {
                   /*test code ------> start*/
                    ...
                    struct ST_TEST * test_ptr = NULL;

                    //if(sr_test_shm.adr == NULL)
                        //return NG;

                    test_ptr = (struct ST_TEST *)sr_test_shm.adr;

                    test_ptr->file_count = file_count;
                    test_ptr->control_retry = control_retry;
                    test_ptr->data_retry = data_retry;
                    test_ptr->recv_timeout_retry = recv_timeout_retry;
                    test_ptr->compare_retry = compare_retry;
                    test_ptr->dir_size = dir_size;
    ...
    else if(shm_category == STASHM)  /*process status*/
    {
        /*set prcess status */
 ...
 *   RETURN VALUE:
 *       exit command should be executed , return EXITCMD.
 *       copy command should be executed , return COPYCMD.
 *       cancel command should be executed, return CANCELCMD.
 *       copy command and cancel command appear at once time,return INVALIDCMD.
 *       can't execute command , return INVALIDCMD.
 *       otherwise , return INVALIDCMD.
 *   DESCRIPTION:
...
estimate_cmd(struct ST_SHM * ptr)
{
       ...
       ((ptr->copy_cmd).copy_re.excute_flag == 0))
    {
        ...
        if(fileget == 1) /*FILEGET_thread is running*/
            return INVALIDCMD;
        ...
        /*copy command and cancel command appear at once time*/
