 ...
 *            memory address
 *   DESCRIPTION:
 *          allocate unused space for an object
 *
 ******************************************************************/
char*
see_malloc(int size)
...
accept_list(struct ST_DATASOCK * datasock_ptr, char* dir_name, struct list * head)
{
    ...
    while(flag == 0)
    {
        ...
        if(rev == 0)
        {
            ...
            /*save list information to memory*/
            if(head->next == NULL)
            {
                head->next = s;
                ...
                else
                {
                    ...
                    {
                        /*save head address of list information memory
                         to another memory*/
                        m = head_list.next;
                        while(head_flag == 0)
                        {
                                        ...
                                        char* dir_name, struct list * head)
{
    ...
    for(retry_ind = 0; retry_ind <= tcp_retry;)
    {
        ...
        if(rev)
        {
            close(data_sock.sid_data);
            data_sock.sid_data = 0;
            if(rev == 0x19) /*execute LIST command failure*/
            {
                retry_ind++;
                continue;
            }
...
analyse_list(struct COPY_DATA * copy_ptr, struct ST_SOCK * sock_ptr, struct list * head, char* pwd)
{
    ...
    while(p != NULL)
    {
        ...
        if(p->data[0] == 'd')
        {
            ...
            /*get file name from LIST information*/
        ...
        else if(p->data[0] == '-')
        {
            ...
            {
                /*save files information to file_list*/
