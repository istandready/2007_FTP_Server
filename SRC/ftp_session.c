 ...
 *   RETURN VALUE:
 *               0:reply code is right code
 *              -1:reply code is not right code
 *            0x29:process error
...
check_reply_code(int reply_code, unsigned char flag)
{
    ...
    write_log("[ERROR INFO] parameter error occured in check_reply_code()\n");
 ...
 *            response : Ftp server response msg
 *   RETURN VALUE:
 *               0    : Read Ftp server response msg success
 ...
 *   RETURN VALUE:
 *               0: creat data connection success
 *          other : creat data connection fail
 *   DESCRIPTION:
...
accept_file(struct ST_DATASOCK * datasock_ptr, char* filename, unsigned short length, int flag)
{
    ...
    datasock_ptr->savefd = open(pathname, O_CREAT | O_TRUNC | O_RDWR);
...
creat_socket(struct COPY_DATA * copy_ptr, struct ST_SOCK * sock_ptr)
{
    ...
    (sock_ptr->sin).sin_port = htons(copy_ptr->port);
 ...
 *          other : execute failure
 *   DESCRIPTION:
 *          traversal directory and get all files
 *
 ******************************************************************/
int
