 ...
 *   RETURN VALUE:
 *               OK   : The successful implementation of cwd command
 ...
 *            pwd     :pass word
 *   RETURN VALUE:
 *               OK   : Send pass word to Ftp server success
...
pass_cmd(struct ST_SOCK * sock_ptr, char* pwd)
{
    ...
    {
        if(rev == NG)
            return 0x23; /* login FTP Server NG */
        return rev;
    }
 ...
 *            username:  user name
 *   RETURN VALUE:
 *               OK   :  Execute User command success
 ...
 *            sock_ptr:socket resource
 *   RETURN VALUE:
 *             OK :  execute list  command success
 ...
 *            filename:file name
 *   RETURN VALUE:
 *            OK  :  execute retr command success
